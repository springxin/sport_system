				...
				<view class="carrier">
					<view class="repair-wrapper">
						<view class="title in1line">
							<template>食物名称</template>
							：
							<template>{{item.footName}}</template>
						</view>
						<view class="content in1line" style="font-size: 30rpx;">
							<template>热量（千卡）</template>
							：
							{{item.heat}}
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			getList() {
				let that = this
				requests.getStock(that.searchParameters).then(res => {
					if (res && res.code === 200) {
