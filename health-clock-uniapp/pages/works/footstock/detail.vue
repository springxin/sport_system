<template>
	<view class="page">
		...
		<button class="confirm-btn" v-if="isNextStep" :class="'bg-' + themeColor.name" :disabled="btnLoading" :loading="btnLoading"
			@tap="nextStep">
			修改
		</button>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			if(this.repairDetail.userId === userId){
				this.isNextStep = true
			}
			this.sendDate.id = this.repairDetail.id
			this.sendDate.brandName = this.repairDetail.brandName
			this.sendDate.footName = this.repairDetail.footName
			this.sendDate.heat = this.repairDetail.heat
			this.getDicts()
		...
		methods: {
				...
				requests.getDicts('foot_type').then(res => {
					if (res && res.code === 200) {
						...
						that.worksTypeList.forEach((item) => {
							...
							// 选中类型
							if(Number(that.repairDetail.footType) === Number(item.id)){
								that.mtype = item.name
								that.sendDate.footType = item.id
							}
						})
					}
			...
			async nextStep() {
				...
				requests.editStock(obj).then(res => {
					if(res && res.code === 200){
						uni.showToast({
							title: res.msg,
							icon: 'success',
