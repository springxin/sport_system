...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			this.sendDate.exerciseType = this.repairDetail.exerciseType
			this.sendDate.eatFoot = this.repairDetail.eatFoot
			this.sendDate.intakeCalorie = this.repairDetail.intakeCalorie
			this.sendDate.consumeCalorie = this.repairDetail.consumeCalorie
			this.getList()
		...
		methods: {
				...
				requests.getExercise(this.searchParameters).then(res => {
					if (res && res.code === 200) {
						...
						that.worksTypeList.forEach((item) => {
							...
							// 选中类型
							if(Number(that.repairDetail.exerciseType) === Number(item.id)){
								that.mtype = item.name
								that.sendDate.exerciseType = item.id
							}
						})
					}
			...
			async nextStep() {
				...
				requests.editClock(obj).then(res => {
					if(res && res.code === 200){
						uni.showToast({
							title: res.msg,
							icon: 'success',
