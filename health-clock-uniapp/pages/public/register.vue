			...
			<view class="wrapper">
				...
				<view class="input-content">
					...
					<view class="input-item">
						...
						<input name="password2" type="password" v-model="form.password2" placeholder="请输入确认密码:" />
					</view>
		...
		methods: {
			...
			register() {
				...
				if (that.form.username === null || that.form.username === '') {
					...
					return;
				} else if (that.form.password2 === null || that.form.password2 === '') {
					uni.showToast({
						icon: 'none',
						title: '确认密码不能为空！'
					});
					return;
				}else if (that.form.password !== that.form.password2) {
					uni.showToast({
						icon: 'none',
						title: '两次输入的密码不一致！'
					});
				...
				requests.register(that.form).then(res => {
					if(res && res.code === 200){
						uni.redirectTo({
							url: './login',
						})
